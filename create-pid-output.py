...
def status_update(position, velocity, angle, angle_vel, thrust, time, lin_targets=None):
    ...
    for i in range(3):
        print(
            f"{position[i, 0]}".ljust(25) + f"{velocity[i, 0]}".rjust(25)
        )
    print()
...
if __name__ == "__main__":
    ...
    lab_lin_vel = np.array([[0], [0], [0]]) # NOTE velocity input doesn't seem to have any impact

    drone_angle = np.array([[1], [-0.5], [0]])
